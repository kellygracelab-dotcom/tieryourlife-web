...
describe("readEnv", () => {
  it("treats empty and blank values as absent", () => {
    expect(readEnv(env({ VITE_RECAPTCHA_SITE_KEY: "" }), "  ")).toEqual({
  ...
  it("trims what is set and reports the dev flag", () => {
    expect(readEnv(env({ DEV: true, VITE_RECAPTCHA_SITE_KEY: " key " }), "t")).toEqual({
  ...
  it("reads Vite's environment and the dev server's token when nothing is passed", () => {
    expect(readEnv()).toMatchObject({ isDev: import.meta.env.DEV });
    expect(typeof __APPCHECK_DEBUG_TOKEN__).toBe("string");
  });
});
